...
def z(s):
	...
	if 'a' in c and 'b' in c:
		a=c['a'].value
		if a in us and c['b'].value in [s[0] for s in us[a][1] if s[1] > time.time()]:
			return (us[a],a)
	return ('','')
w=lambda s,b:s.wfile.write(bytes(c('<title>TODOZtitle><body]max-width:800px;W:auto;padding:16px">%sZbody>'%b),d))
def c(s,b=0):
	if b > 5:
		return s
	for e in zip(
